...
interface InfiniteScrollElementSelectProps {
    value?: string;
    onValueChange?: (value: string) => void;
    placeholder?: string;
    className?: string;
    initialData?: { id: string; name: string; tag: string } | null;
}

export function InfiniteScrollElementSelect({
    value,
    onValueChange,
    placeholder = "Select an element...",
    className,
    initialData,
}: InfiniteScrollElementSelectProps) {
    ...
    const fetchElements = useCallback(async (pageNum: number, search: string = '', force: boolean = false) => {
        ...
        try {
            ...
            if (data && data.length > 0) {
                ...
                setHasMore(false);
                if (pageNum === 0) {
                    setElements(initialData && initialData.id === value ? [initialData] : []);
                }
            }
        } catch (err) {
    ...
    useEffect(() => {
        ...
        const observer = new IntersectionObserver(
            (entries) => {
                if (entries[0].isIntersecting && hasMore && !isLoading && elements.length > 0) {
                    const nextPage = page + 1;
                    setPage(nextPage);
                    fetchElements(nextPage, searchTerm);
                }
            },
            { threshold: 1.0 }
        );
    ...
    }, [open, hasMore, isLoading, page, fetchElements, searchTerm, elements.length]);
    ...
    // Use initialData to populate selected element if not found in list
    useEffect(() => {
        if (initialData && value === initialData.id) {
            setElements(prev => {
                if (prev.find(e => e.id === initialData.id)) return prev;
                return [initialData, ...prev];
            });
        }
    }, [initialData, value]);

    const selectedElement = elements.find((element) => element.id === value);
            ...
            <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-0" side="bottom" align="start" sideOffset={5} avoidCollisions={false}>
                <Command shouldFilter={false}>
                    ...
                    <CommandList className="max-h-[200px]">
                        <CommandEmpty>
                            {isLoading ? "Loading..." : "No elements found."}
                        </CommandEmpty>
                        <CommandGroup>
